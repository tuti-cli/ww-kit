...
- Check `.ww-kit/ARCHITECTURE.md` (if present) for boundary/dependency alignment issues.
- Check `.ww-kit/RULES.md` (if present) for explicit convention violations.
- Check `.ww-kit/ROADMAP.md` (if present) for milestone alignment and mention missing linkage for likely `feat`/`fix`/`perf` work.
...
`/ww-review` is read-only for context artifacts by default. Do not modify context files unless user explicitly asks.

### Project Context

**Read `.ww-kit/skill-context/ww-review/SKILL.md`** — MANDATORY if the file exists.
...
**User:** `/ww-review https://github.com/org/repo/pull/123`
Review PR from URL.
