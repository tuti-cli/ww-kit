---
name: ww-improve
description: Refine and enhance an existing implementation plan with a second iteration. Re-analyzes the codebase, checks for gaps, missing tasks, wrong dependencies, and improves the plan quality. Use after /ww-plan to polish the plan before implementation, or to improve an existing /ww-fix plan.
...
4. Print availability summary and usage hints:
   - /ww-improve @<path> <optional prompt>
   - /ww-improve <optional prompt>      # automatic priority
5. If none found, suggest creating a plan via /ww-plan or /ww-fix
6. STOP.
```
...
2. No explicit `@<path>` override → Check current git branch:
   ...
   → Look for .ww-kit/plans/<branch-name>.md (from /ww-plan full)
   Example: feature/user-auth → .ww-kit/plans/feature-user-auth.md
3. No branch-based plan → Check .ww-kit/PLAN.md (from /ww-plan fast)
4. No branch-based plan and no .ww-kit/PLAN.md → Check .ww-kit/FIX_PLAN.md (from /ww-fix plan mode)
```
...
- /ww-plan full <description>  — for a new feature (creates branch + plan)
- /ww-plan fast <description>  — for a quick task plan
- /ww-fix <bug description>    — for a bugfix plan (.ww-kit/FIX_PLAN.md)
```
...
Read `.ww-kit/DESCRIPTION.md` if it exists:
- Tech stack
- Architecture
- Conventions
...
- If `.ww-kit/skill-context/ww-improve/SKILL.md` does not exist and `.ww-kit/patches/` exists:
...
**Read `.ww-kit/skill-context/ww-improve/SKILL.md`** — MANDATORY if the file exists.
...
Now do a **deeper** codebase exploration than what `/ww-plan` did initially:
...
→ Found plan: .ww-kit/plans/feature-user-auth.md
→ 6 tasks in plan
...
```
User: /ww-improve добавь обработку ошибок и валидацию входных данных

→ Found plan: .ww-kit/PLAN.md
→ 4 tasks in plan
...
→ Branch: main
→ No .ww-kit/plans/main.md found
→ No .ww-kit/PLAN.md found
→ No .ww-kit/FIX_PLAN.md found
→ No plan file found

"No active plan found. Create one first:
- /ww-plan full <description>
- /ww-plan fast <description>
...
```
User: /ww-improve @my-custom-plan.md add rollback and edge-case handling
...
```
User: /ww-improve --list

## Available Plans
Current branch: feature/user-auth
- [x] .ww-kit/plans/feature-user-auth.md
- [ ] .ww-kit/PLAN.md
- [x] .ww-kit/FIX_PLAN.md

Use:
- /ww-improve @.ww-kit/plans/feature-user-auth.md
- /ww-improve add validation and retries
```
...
→ Found plan: .ww-kit/plans/feature-product-search.md
→ 5 tasks in plan
