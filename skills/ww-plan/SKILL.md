...
argument-hint: "[fast | full | --deep] [--parallel | --list | --cleanup <branch>] <description>"
...
- **Fast** — quick plan, no git branch, saves to `.ww-kit/PLAN.md`
- **Full** — creates git branch, asks preferences, saves to `.ww-kit/plans/<branch>.md`
- **Deep** (`--deep`) — structured discovery: interview → stories → phases → tasks. For greenfield features or when requirements are unclear. Runs a business discovery interview first, then generates user stories grouped into phases before creating the task breakdown.

## Workflow

### Step 0: Load Project Context

**FIRST:** Read `.ww-kit/DESCRIPTION.md` if it exists to understand:
- Tech stack (language, framework, database, ORM)
- Project architecture
- Coding conventions
- Non-functional requirements

**ALSO:** Read `.ww-kit/ARCHITECTURE.md` if it exists to understand:
...
- **Follow architecture guidelines from `.ww-kit/ARCHITECTURE.md` when planning file structure and task organization**

**Read `.ww-kit/skill-context/ww-plan/SKILL.md`** — MANDATORY if the file exists.
...
**OPTIONAL (recommended):** Read `.ww-kit/ROADMAP.md` if it exists:
- Use it to link this plan to a specific milestone (when applicable)
- This reduces ambiguity in `/ww-do` milestone completion and `/ww-verify` roadmap gates

**OPTIONAL (recommended):** Read `.ww-kit/RESEARCH.md` if it exists:
- Treat `## Active Summary (input for /ww-plan)` as an additional requirements source
...
--deep      → Deep planning mode: discovery → stories → phases → tasks
...
**Parsing rules:**
- Strip `--parallel`, `--deep`, `--list`, `--cleanup <branch>`, `fast`, `full` from `$ARGUMENTS`
- Remaining text becomes the description
- `--list` and `--cleanup` execute immediately and **STOP** (do NOT continue to Step 1+)

**If the description is empty:**
- If `.ww-kit/RESEARCH.md` exists and its `Active Summary` has a non-empty `Topic:`, default the description to that topic (no extra user input required)
- Otherwise, ask the user for a short feature description

**If `--deep` is present**, jump to [Deep Planning Mode](#deep-planning-mode).
...
If the user did not provide a description and `.ww-kit/RESEARCH.md` exists:
...
- If `.ww-kit/DESCRIPTION.md` already provides sufficient context, this step can be skipped
...
4. Roadmap milestone linkage (only if `.ww-kit/ROADMAP.md` exists):
   a. Link this plan to a milestone
   b. Skip — no linkage (allowed; `/ww-verify --strict` should report WARN, not fail, for missing linkage alone)
...
Store all preferences — they will be used in the plan file and passed to `/ww-do`.

Docs policy semantics:
- `Docs: yes` → `/ww-do` MUST show a mandatory documentation checkpoint and route docs changes through `/wws-docs`
- `Docs: no` (or unset) → `/ww-do` emits `WARN [docs]` and continues without a mandatory docs checkpoint

**If `.ww-kit/ROADMAP.md` exists and the user chose milestone linkage:**
- Read `.ww-kit/ROADMAP.md` and list candidate milestones (prefer unchecked items)
...
# Ensure ww-kit directories exist before copy operations
mkdir -p "${WORKTREE}/.ww-kit"
mkdir -p "${WORKTREE}/.ww-kit/plans"
mkdir -p "${WORKTREE}/.ww-kit/patches"
mkdir -p "${WORKTREE}/.ww-kit/evolutions"

# Project context
cp .ww-kit/DESCRIPTION.md "${WORKTREE}/.ww-kit/DESCRIPTION.md" 2>/dev/null
cp .ww-kit/ARCHITECTURE.md "${WORKTREE}/.ww-kit/ARCHITECTURE.md" 2>/dev/null
cp .ww-kit/RESEARCH.md "${WORKTREE}/.ww-kit/RESEARCH.md" 2>/dev/null

# Skill-context (primary learning context)
cp -r .ww-kit/skill-context/ "${WORKTREE}/.ww-kit/skill-context/" 2>/dev/null
...
# Copying the evolve cursor without the full patch history can cause /ww-evolve to skip patches
# or trigger a partial rescan.

# Limited patch fallback: copy only recent patches (latest 10 by filename)
for patch in $(ls -1 .ww-kit/patches/*.md 2>/dev/null | sort | tail -n 10); do
  cp "${patch}" "${WORKTREE}/.ww-kit/patches/"
done
...
3. Roadmap milestone linkage (only if `.ww-kit/ROADMAP.md` exists):
   a. Link this plan to a milestone
   b. Skip — no linkage (allowed; `/ww-verify --strict` should report WARN, not fail, for missing linkage alone)
```

**Plan file:** Always `.ww-kit/PLAN.md` (no branch, no branch-named file).
...
- **Fast mode** → `.ww-kit/PLAN.md`
- **Full mode** → `.ww-kit/plans/<branch-name>.md` (replace `/` with `-`)

**Before saving, ensure directory exists:**
```bash
mkdir -p .ww-kit/plans  # only when saving to branch-named plan files
```
...
- `Roadmap Linkage` section (optional, only if `.ww-kit/ROADMAP.md` exists)
- `Research Context` section (optional, if `.ww-kit/RESEARCH.md` exists)
- `Tasks` section grouped by phases
- `Commit Plan` section when there are 5+ tasks

If `.ww-kit/ROADMAP.md` exists:
...
**Full mode + parallel (`--parallel`):** Automatically invoke `/ww-do` — the whole point of parallel is autonomous end-to-end execution in an isolated worktree.

```
/ww-do

CONTEXT FROM /ww-plan:
- Plan file: .ww-kit/plans/<branch-name>.md
- Testing: yes/no
...
1. Check if `<worktree>/.ww-kit/plans/` contains any plan files
...
8. **Plan file location** — Fast mode: `.ww-kit/PLAN.md`. Full mode: `.ww-kit/plans/<branch-name>.md`
9. **Ownership boundary** — This command owns plan files only (`.ww-kit/PLAN.md`, `.ww-kit/plans/<branch>.md`). Use owner commands (`/wws-roadmap`, `/ww-rules`, `/ww-explore`) for their artifacts.
10. **Roadmap linkage (when available)** — If `.ww-kit/ROADMAP.md` exists, include a `## Roadmap Linkage` section in the plan (or explicitly state it was skipped).

## Plan File Handling

**Fast mode (`.ww-kit/PLAN.md`)**
- Temporary plan for quick work
- `/ww-do` may offer deletion after completion

**Full mode (`.ww-kit/plans/<branch>.md`)**
...
When `--deep` is present, run a structured discovery pipeline before creating tasks. This is for greenfield features or unclear requirements.

### Phase 1: Discovery Interview

Conduct a focused business discovery interview. Ask one question at a time, covering:

1. **What** — What are we building? What problem does it solve?
2. **Who** — Who are the users? What are their roles?
3. **Why** — Why now? What's the business value?
4. **How** — Any technical constraints? Integration requirements?
5. **Scope** — What's in scope vs out of scope for this iteration?

Save discovery notes to `.ww-kit/deep-plan-discovery.md`.

### Phase 2: User Stories

From the discovery, generate user stories in the format:
```
As a [role], I want [action], so that [benefit].
```

Group by domain area. Assign priority (must-have / should-have / nice-to-have).
Present to user for approval. Remove or adjust based on feedback.

### Phase 3: Phases

Group approved stories into implementation phases:
- **Phase 1:** Foundation / MVP (must-haves + critical infrastructure)
- **Phase 2:** Core features (remaining must-haves + key should-haves)
- **Phase 3:** Enhancement (remaining should-haves + nice-to-haves)

### Phase 4: Task Breakdown

For Phase 1 stories, generate the detailed task breakdown following the standard plan format (Step 1+). Save as a full plan using the selected mode (fast or full, defaults to full).

Present summary:
```
Deep planning complete!

Discovery: .ww-kit/deep-plan-discovery.md
Plan: .ww-kit/plans/<branch>.md (or PLAN.md)

Phase 1: N tasks (ready to execute)
Phase 2: M stories (planned, not yet broken into tasks)
Phase 3: K stories (backlog)

Run /ww-do to start executing Phase 1.
```
