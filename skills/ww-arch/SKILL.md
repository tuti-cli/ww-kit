---
name: ww-arch
description: Generate architecture guidelines for the project. Analyzes tech stack from DESCRIPTION.md, recommends an architecture pattern, and creates .ww-kit/ARCHITECTURE.md. Use when setting up project architecture, asking "which architecture", or after /ww setup.
argument-hint: "[clean|ddd|microservices|monolith|layers] or [brainstorm|challenge|decide <topic>]"
...
Generate `.ww-kit/ARCHITECTURE.md` with architecture decisions tailored to the project.
...
**If `.ww-kit/DESCRIPTION.md` does not exist:**
```
⚠️  No project description found.

Run /ww first to set up project context, or describe your project manually:
...
**Read `.ww-kit/skill-context/ww-arch/SKILL.md`** — MANDATORY if the file exists.
...
**If `$ARGUMENTS` specifies an architecture** (e.g., `/ww-arch clean`):
...
### Step 2: Generate .ww-kit/ARCHITECTURE.md

```bash
mkdir -p .ww-kit
```

Generate `.ww-kit/ARCHITECTURE.md` with the following structure, **adapted to the project's tech stack and language**:
...
If `.ww-kit/DESCRIPTION.md` exists, add an `## Architecture` section (or update if it already exists):
...
If `AGENTS.md` exists in the project root, add `.ww-kit/ARCHITECTURE.md` to the "AI Context Files" table:
...
All workflow skills (/ww-plan, /ww-do) will now follow these architecture guidelines.
```
...
For specific architecture decisions (not initial setup), use the brainstorm/challenge/decide flow:

- `/ww-arch brainstorm <topic>` — explore architecture options
- `/ww-arch challenge <topic>` — stress-test a proposal using `/ww-grill`
- `/ww-arch decide <topic>` — lock in decision, write ADR

### Brainstorm

1. Read `.ww-kit/DESCRIPTION.md` and `.ww-kit/ARCHITECTURE.md` for context
2. Read existing ADRs in `.ww-kit/decisions/`
3. Generate 2-3 architecture options with tradeoffs and risks
4. Write proposal to `.ww-kit/proposals/{topic}.md`

### Challenge

1. Read `.ww-kit/proposals/{topic}.md` (if no proposal, tell user to brainstorm first)
2. Use `/ww-grill` to stress-test the proposal:
   - Phase 1: clarify assumptions
   - Phase 2: find failure modes, hidden costs
   - Phase 3: test against scale, debugging, rollback
3. Write challenge findings to `.ww-kit/challenges/{topic}.md`

### Decide

1. Read proposal + challenge findings, present summary
2. Ask user which option and rationale
3. Write ADR to `.ww-kit/decisions/NNN-{topic}.md`:
...
4. Update `.ww-kit/ARCHITECTURE.md` with the decision if relevant

---

## Artifact Ownership

- Primary ownership: `.ww-kit/ARCHITECTURE.md`.
- Allowed companion updates: architecture pointer in `.ww-kit/DESCRIPTION.md`, architecture row in `AGENTS.md` context table.
