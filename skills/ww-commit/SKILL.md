...
**Read `.ww-kit/skill-context/ww-commit/SKILL.md`** — MANDATORY if the file exists.
...
2. **Run Context Gates (Read-Only)**
   - Check `.ww-kit/ARCHITECTURE.md` and `.ww-kit/DESCRIPTION.md` (if present) to catch obvious scope/boundary drift
   - Check `.ww-kit/RULES.md` and `.ww-kit/ROADMAP.md` (if present) to catch rule and milestone alignment issues
...
If argument provided (e.g., `/ww-commit auth`):
- Use it as the scope
...
- `/ww-commit` has no implicit strict mode — context gates are warning-first unless user explicitly requests blocking behavior
- Treat `.ww-kit/ARCHITECTURE.md`, `.ww-kit/ROADMAP.md`, `.ww-kit/RULES.md`, and `.ww-kit/DESCRIPTION.md` as read-only context in this command
