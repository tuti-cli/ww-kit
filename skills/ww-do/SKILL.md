...
- `skills/ww-do/references/IMPLEMENTATION-GUIDE.md` → "List Available Plans (`--list`)"
...
Options:
1. Yes, commit now (/ww-commit)
2. No, stash and continue
3. Cancel
```

**Based on choice:**
- Yes → run `/ww-commit`, then continue to plan discovery
- No → `git stash push -m "ww-do: stash before plan execution"`, then continue
- Cancel → inform the user: "Implementation cancelled." → **STOP**

**If NO plan file exists but `.ww-kit/FIX_PLAN.md` exists:**

A fix plan was created by `/ww-fix` in plan mode. Redirect to fix workflow:

```
Found a fix plan (.ww-kit/FIX_PLAN.md).

This plan was created by /ww-fix and should be executed through the fix workflow
(it creates a patch and handles cleanup automatically).

Running /ww-fix to execute the plan...
```

→ **Invoke `/ww-fix`** (without arguments — it will detect FIX_PLAN.md and execute it).
...
**Based on choice:**
- New feature from current → `/ww-plan full <description>`
- Return to main → `git checkout main`, then `git pull` → `/ww-plan full <description>`
- Quick task → `/ww-plan fast <description>`
...
**Read `.ww-kit/DESCRIPTION.md`** if it exists to understand:
- Tech stack (language, framework, database, ORM)
- Project architecture and conventions
- Non-functional requirements

**Read `.ww-kit/ARCHITECTURE.md`** if it exists to understand:
...
**Read `.ww-kit/RULES.md`** if it exists:
...
**Read `.ww-kit/skill-context/ww-do/SKILL.md`** — MANDATORY if the file exists.
...
- If `.ww-kit/skill-context/ww-do/SKILL.md` does not exist and `.ww-kit/patches/` exists:
  - Use `Glob` to find `*.md` files in `.ww-kit/patches/`
...
3. If file does not exist:
   "Plan file not found: <path>
    ...
    - /ww-do @.ww-kit/PLAN.md
    - /ww-do @.ww-kit/plans/feature-user-auth.md"
   → STOP
4. If file is .ww-kit/FIX_PLAN.md:
   → invoke /ww-fix (ownership + cleanup workflow) and STOP
...
1. Check current git branch:
   ...
   → Look for .ww-kit/plans/<branch-name>.md (e.g., feature/user-auth → .ww-kit/plans/feature-user-auth.md)
2. No branch-based plan → Check .ww-kit/PLAN.md
3. No branch-based plan and no .ww-kit/PLAN.md → Check .ww-kit/FIX_PLAN.md
   → If exists: invoke /ww-fix (handles its own workflow with patches) and STOP
```

**Priority:**
1. `@<path>` argument - explicit user-selected plan file
2. Branch-named file (from `/ww-plan full`) - if it matches current branch
3. `.ww-kit/PLAN.md` (from `/ww-plan fast`) - fallback when no branch-based plan exists
4. `.ww-kit/FIX_PLAN.md` - redirect to `/ww-fix` (from `/ww-fix` plan mode)
...
**3.7: Update .ww-kit/DESCRIPTION.md if needed**
...
→ Update `.ww-kit/DESCRIPTION.md` to reflect the change:
...
This keeps .ww-kit/DESCRIPTION.md as the source of truth.
...
→ Update `.ww-kit/ARCHITECTURE.md` — if new modules or layers were added that should be documented in the folder structure section.
...
**Based on choice:**
- Yes, commit now → invoke `/ww-commit` with the suggested message, then continue to next task
- No, continue to next task → proceed to the next task without committing
- Skip all commit checkpoints → for all subsequent checkpoints within this `/ww-do` run, skip the prompt automatically and proceed directly to the next task (as if user selected "No, continue to next task" each time). This is in-context memory — resets on `/clear` or new session
...
Plan file: .ww-kit/plans/feature-product-search.md
Files modified:
...
1. 🔍 /ww-verify — Verify nothing was missed (recommended)
2. 💾 /ww-commit — Commit the changes directly
```
...
- If this plan introduced new dependencies/integrations or changed the stack, update `.ww-kit/DESCRIPTION.md` with factual deltas only.
- Do not rewrite unrelated sections.

**ARCHITECTURE.md + AGENTS.md (allowed in this command):**
- If new modules/layers/folders were added (or dependency rules changed), update `.ww-kit/ARCHITECTURE.md` to reflect the new structure and constraints.
...
- If milestone mapping is ambiguous, emit `WARN [roadmap] ...` and suggest the owner command:
  - `/wws-roadmap check`
  - or `/wws-roadmap <short update request>`

**RULES.md (NOT allowed in this command):**
- Never edit `.ww-kit/RULES.md` from `/ww-do`.
- If you discovered repeatable conventions/pitfalls during implementation, propose up to 3 candidate rules and ask the user to add them via `/ww-rules`.
- Do not invoke `/ww-rules` automatically (it is user-invoked).

If candidate rules exist:

```
AskUserQuestion: Capture new project rules in `.ww-kit/RULES.md`?

Options:
1. Yes — output `/ww-rules ...` commands (recommended)
2. No — skip
```
...
Options:
1. Update existing docs (recommended) — invoke /wws-docs
2. Create a new feature doc page — invoke /wws-docs with feature-page context
3. Skip documentation
```

Handling:
- Option 1 → invoke `/wws-docs` to update README/docs based on completed work
- Option 2 → invoke `/wws-docs` with context to create `docs/<feature-slug>.md`, include sections (Summary, Usage/user-facing behavior, Configuration, API/CLI changes, Examples, Troubleshooting, See Also), and add a README docs-table link
- Option 3 → do not invoke `/wws-docs`; emit `WARN [docs] Documentation skipped by user`

If plan setting is `Docs: no` or setting is unset:
- Do **not** show a mandatory docs checkpoint prompt
- Do **not** invoke `/wws-docs` automatically
...
- **If `.ww-kit/PLAN.md`** (from `/ww-plan fast`):
  ```
  AskUserQuestion: Would you like to delete .ww-kit/PLAN.md? (It's no longer needed)
...
- **If branch-named file** (e.g., `.ww-kit/plans/feature-user-auth.md`):
...
- "No, I'll handle it manually" → show a reminder:
  ...
  To merge and clean up later:
    cd <main-repo-path>
    git merge <branch>
    /ww-plan --cleanup <branch>
  ```

#### Worktree Merge

1. **Ensure everything is committed** — check `git status`. If uncommitted changes exist, suggest `/ww-commit` first and wait.
...
Options:
1. Verify first — Run /ww-verify to check completeness (recommended)
2. Skip to commit — Go straight to /ww-commit
```

**Based on choice:**
- "Verify first" → invoke `/ww-verify` → after it completes, continue to context cleanup below
- "Skip to commit" → invoke `/ww-commit` → after it completes, continue to context cleanup below
...
/ww-do --list
```
Lists `.ww-kit/PLAN.md`, `.ww-kit/FIX_PLAN.md`, and current-branch `.ww-kit/plans/<branch>.md` (if present), then exits without implementation.

### Use Explicit Plan File
```
/ww-do @my-custom-plan.md
/ww-do @.ww-kit/plans/feature-user-auth.md status
```
...
- ✅ Follow `/wws-best-practices` guidelines (naming, structure, error handling)
...
- ❌ Violate `.ww-kit/ARCHITECTURE.md` conventions for file placement and module boundaries

## Artifact Ownership Boundaries

- Primary ownership in this command: task execution state and plan progress checkboxes.
- Allowed context artifact updates: `.ww-kit/DESCRIPTION.md`, `.ww-kit/ARCHITECTURE.md`, and roadmap milestone completion in `.ww-kit/ROADMAP.md` when implementation evidence justifies it.
- Read-only context in this command by default: `.ww-kit/RULES.md`, `.ww-kit/RESEARCH.md`.
