...
Then derive:
- `branchPlan = .ww-kit/plans/<branch-with-slashes-replaced-by-hyphens>.md`
- `fastPlan = .ww-kit/PLAN.md`
- `fixPlan = .ww-kit/FIX_PLAN.md`
...
Use:
- /ww-do @<path>  to execute a specific plan
- /ww-do          to use automatic priority
```
...
Then:
- Re-open the active plan file (`@plan-file` override if provided; otherwise branch plan first, then `PLAN.md`, then `FIX_PLAN.md` redirect to `/ww-fix`).
...
```
Session 1:
  /ww-plan full Add user authentication
...
Session 2:
  ...
  → Reads plan: .ww-kit/plans/feature-user-authentication.md
  → Loads state: 3/6 complete
  → Continues from task #4
  → Completes tasks #4, #5, #6
  → All done, suggests /ww-commit
```
