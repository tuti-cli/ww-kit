---
name: wws-roadmap
description: Create or update a project roadmap with major milestones. Generates .ww-kit/ROADMAP.md — a strategic checklist of high-level goals. Use when user says "roadmap", "project plan", "milestones", or "what to build next".
...
**Read `.ww-kit/skill-context/wws-roadmap/SKILL.md`** — MANDATORY if the file exists.
...
Otherwise check if `.ww-kit/ROADMAP.md` exists:
...
Create `.ww-kit/ROADMAP.md` with this format:
...
- Each milestone is a **high-level goal**, not a granular task (that's `/ww-plan`)
...
Apply changes if requested, then save to `.ww-kit/ROADMAP.md`.
...
- Read `.ww-kit/ROADMAP.md`
- Read `.ww-kit/DESCRIPTION.md` for context
...
- Update `.ww-kit/ROADMAP.md`

**2.5: Reprioritize (if chosen)**

- Show current order
- Ask user for new order or let them describe priority changes
- Reorder milestones in `.ww-kit/ROADMAP.md`

**2.6: Save Changes**

Update `.ww-kit/ROADMAP.md` with all modifications.
...
/ww-plan <milestone description>  → creates branch + plan
/ww-do                     → executes the plan
```

---

### Mode 3: Check Progress (`/wws-roadmap check`)

Automated scan — analyze the codebase and mark completed milestones without interactive questions.

**Requires** `.ww-kit/ROADMAP.md` to exist. If it doesn't — tell the user to run `/wws-roadmap` first.

**3.1: Read roadmap and project context**

- Read `.ww-kit/ROADMAP.md`
- Read `.ww-kit/DESCRIPTION.md` for tech stack context
...
5. **NO implementation** — this skill only plans, use `/ww-plan` to start a feature and `/ww-do` to execute
6. **Ownership boundary** — this command owns roadmap structure/content; `/ww-do` may only mark milestones completed when implementation evidence is clear
