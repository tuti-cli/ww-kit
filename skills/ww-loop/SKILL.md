...
- `.ww-kit/DESCRIPTION.md`
- `.ww-kit/ARCHITECTURE.md`
- `.ww-kit/RULES.md`

Use them to keep outputs aligned with project conventions.

**Read `.ww-kit/skill-context/ww-loop/SKILL.md`** — MANDATORY if the file exists.
...
- **`list`**: scan `.ww-kit/evolution/` directories, read each `run.json`, display table of `alias | status | iteration | last_score | updated_at`
...
3. Ask user where to save the final artifact (default: keep in `.ww-kit/evolution/<alias>/artifact.md`)
4. Offer to copy artifact to a user-specified path
5. Suggest next skills based on artifact type:
   - API spec -> `/ww-plan` to implement it
   - Code -> `/ww-verify` to check it
   - Docs -> `/wws-docs` to integrate it
...
```text
💡 Context is growing. Recommended: /clear then /ww-loop resume
...
/ww-loop resume
/ww-loop resume courses-api-ddd
/ww-loop status
/ww-loop stop
/ww-loop list
/ww-loop history
