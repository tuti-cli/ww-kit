...
description: >-
  ...
  Use after "/ww-do" completes, or when user says "verify", "check work", "did we miss anything".
...
**This skill is optional** — invoked after `/ww-do` finishes all tasks, or manually at any time.
...
- Treat it as the canonical source for:
  ...
  - read-only behavior for `ww-commit`/`ww-review`/`ww-verify`,
...
```
1. .ww-kit/PLAN.md exists? → Use it
2. No PLAN.md → Check current git branch:
   git branch --show-current
   → Look for .ww-kit/plans/<branch-name>.md
```
...
- Read `.ww-kit/DESCRIPTION.md` for project context (tech stack, conventions)
- Read `.ww-kit/ARCHITECTURE.md` for dependency and boundary rules (if present)
- Read `.ww-kit/RULES.md` for project-specific conventions (if present)
- Read `.ww-kit/ROADMAP.md` for milestone alignment checks (if present)

**Read `.ww-kit/skill-context/ww-verify/SKILL.md`** — MANDATORY if the file exists.
...
Check if `.ww-kit/DESCRIPTION.md` reflects the current state:
...
- **Roadmap gate**
  ...
  - Warn: `.ww-kit/ROADMAP.md` missing, ambiguous mapping, or no milestone linkage for `feat`/`fix`/`perf` scope
...
- Missing milestone linkage for `feat`/`fix`/`perf` remains a warning (even when `.ww-kit/ROADMAP.md` exists).
...
`/ww-verify` is **read-only** for context artifacts. Do not edit or regenerate `.ww-kit/*` files here.
...
- `DESCRIPTION.md` drift → suggest `/ww` (or note that `/ww-do` should have updated it during implementation)
- `ARCHITECTURE.md` drift → suggest `/ww-arch`
- `ROADMAP.md` drift → suggest `/wws-roadmap check` (or `/wws-roadmap <update request>`)
- `RULES.md` drift → suggest `/ww-rules <rule text>`
...
Options:
1. Fix now (recommended) — Use /ww-fix to address all issues
2. Fix critical only — Use /ww-fix for incomplete tasks, skip warnings
3. Fix directly here — Address issues in this session without /ww-fix
4. Accept as-is — Mark everything as done, move on
```

**If "Fix now" or "Fix critical only":**
- First suggest using `/ww-fix` and pass a concise issue summary as argument
- Example:
  - `/ww-fix complete Task #3 password reset email flow, implement Task #8 docs update, remove TODOs in src/services/auth.ts and src/middleware/rate-limit.ts, document SENDGRID_API_KEY in .env.example`
- If user agrees, proceed via `/ww-fix`
- If user declines `/ww-fix`, continue with direct implementation in this session
- For each incomplete/partial task — implement the missing pieces (follow the same implementation rules as `/ww-do`)
...
- If unresolved issues remain (accepted or deferred), suggest `/ww-fix` first
...
1. 🛠️ /ww-fix [issue summary] — Fix remaining verification issues
2. 🔒 /wws-security — Run security audit on the new code
3. 👀 /ww-review — Code review of the implementation
4. 💾 /ww-commit — Commit the changes
...
Options:
1. Fix issues — Run /ww-fix with verification findings
2. Security check — Run /wws-security on changed files
3. Code review — Run /ww-review on the implementation
4. Both — Run security check, then code review
5. Skip — Proceed to commit
```

**If fix issues selected** → suggest invoking `/ww-fix <issue summary>`
**If security check selected** → suggest invoking `/wws-security`
**If code review selected** → suggest invoking `/ww-review`
**If both** → suggest security first, then review
**If skip** → suggest `/ww-commit`
