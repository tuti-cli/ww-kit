...
- `/wws-security` — Full audit checklist
...
- `/wws-security csrf` — Cross-site request forgery
- `/wws-security secrets` — Secrets & credentials
- `/wws-security api` — API security
- `/wws-security infra` — Infrastructure security
- `/wws-security prompt-injection` — LLM prompt injection
- `/wws-security race-condition` — Race conditions & TOCTOU
- `/wws-security ignore <item>` — Ignore a specific check item

## Ignored Items (SECURITY.md)

Before running any audit, **always read** the file `.ww-kit/SECURITY.md` in the project root. If it exists, it contains a list of security checks the team has decided to ignore.

### How ignoring works

**When the user runs `/wws-security ignore <item>`:**

1. Read the current `.ww-kit/SECURITY.md` file (create if doesn't exist)
...
**When running any audit (`/wws-security` or a specific category):**

1. Read `.ww-kit/SECURITY.md` at the start
...
```
⏭️ Ignored Items (from .ww-kit/SECURITY.md)
┌─────────────────┬──────────────────────────────────────┬────────────┐
│ Item            │ Reason                               │ Date       │
├─────────────────┼──────────────────────────────────────┼────────────┤
...
└─────────────────┴──────────────────────────────────────┴────────────┘
⚠️  2 items ignored. Run `/wws-security` without ignores to see full audit.
```
...
**Read `.ww-kit/skill-context/wws-security/SKILL.md`** — MANDATORY if the file exists.
