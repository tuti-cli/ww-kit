...
**Read `.ww-kit/skill-context/wws-skill/SKILL.md`** — MANDATORY if the file exists.
...
```
If not found — ask user for path, offer to skip scan (at their risk), or suggest installing Python. If skipping, still perform Level 2 (manual review). See `/ww` skill for full detection flow.
...
0. Scope check (MANDATORY):
   ...
   - If path points to built-in ww-kit skills ({{skills_dir}}/ww or {{skills_dir}}/ww-*), this is wrong target selection for install-time security checks.
   - Do not block external-skill installation decisions based on scans of built-in ww* skills.
1. Download/fetch the skill content
2. LEVEL 1 — Run automated scan:
   $PYTHON ~/{{skills_dir}}/wws-skill/scripts/security-scan.py <skill-path>
...
- `/wws-skill <name>` - Generate a new skill interactively
- `/wws-skill <url> [url2] [url3]...` - **Learn Mode**: study URLs and generate a skill from them
- `/wws-skill search <query>` - Search existing skills on skills.sh for inspiration
- `/wws-skill scan <path>` - **Security scan**: run two-level security check on a skill
- `/wws-skill validate <path>` - **Full validation**: structure check + two-level security scan
- `/wws-skill template <type>` - Get a template (basic, task, reference, visual)
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   $PYTHON ~/{{skills_dir}}/wws-skill/scripts/security-scan.py <path>
   ```
...
**Trigger:** `/wws-skill validate <path>`
...
7. **AUTO-SCAN**: Run `/wws-skill scan <generated-skill-path>` on the result
...
```bash
$PYTHON ~/{{skills_dir}}/wws-skill/scripts/security-scan.py ./skill-name/
```
