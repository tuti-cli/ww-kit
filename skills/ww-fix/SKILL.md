...
**BEFORE anything else**, check if `.ww-kit/FIX_PLAN.md` exists.

**If the file EXISTS:**
- Read `.ww-kit/FIX_PLAN.md`
...
- After the fix is fully applied and verified, **delete** `.ww-kit/FIX_PLAN.md`:
  ```bash
  rm .ww-kit/FIX_PLAN.md
  ```
- Continue to Step 4 (Verify), Step 5 (Test suggestion), Step 6 (Patch)

**If the file DOES NOT exist AND `$ARGUMENTS` is empty:**
- Tell the user: "No fix plan found and no problem description provided. Please either provide a bug description (`/ww-fix <description>`) or create a fix plan first."
- **STOP.**
...
**Read `.ww-kit/skill-context/ww-fix/SKILL.md`** — MANDATORY if the file exists.
...
- If `.ww-kit/skill-context/ww-fix/SKILL.md` does not exist and `.ww-kit/patches/` exists:
...
Then create `.ww-kit/FIX_PLAN.md` with this structure:
...
Plan saved to `.ww-kit/FIX_PLAN.md`.
...
**User:** `/ww-fix TypeError: Cannot read property 'name' of undefined in UserProfile`
...
**User:** `/ww-fix /api/orders returns empty array for authenticated users`
...
**User:** `/ww-fix email validation accepts invalid emails`
...
11. **Ownership boundary** - `/ww-fix` owns `.ww-kit/FIX_PLAN.md` and `.ww-kit/patches/*.md`; treat `.ww-kit/DESCRIPTION.md`, roadmap/rules/architecture context artifacts as read-only unless the user explicitly requests otherwise
