---
name: ww-rules
description: Add project-specific rules and conventions to .ww-kit/RULES.md. Each invocation appends new rules. These rules are automatically loaded by /ww-do before execution. Use when user says "add rule", "remember this", "convention", or "always do X".
argument-hint: "[rule text or topic]"
allowed-tools: Read Write Edit Glob Grep AskUserQuestion Questions
disable-model-invocation: true
---

# ww-kit Rules - Project Conventions

Add short, actionable rules and conventions for the current project. Rules are saved to `.ww-kit/RULES.md` and automatically loaded by `/ww-do` before task execution.

## Workflow

### Step 0: Load Skill Context

**Read `.ww-kit/skill-context/ww-rules/SKILL.md`** — MANDATORY if the file exists.
...
```
/ww-rules Always use DTO classes instead of arrays
```
...
**Check if `.ww-kit/RULES.md` exists:**

```
Glob: .ww-kit/RULES.md
```
...
> Short, actionable rules and conventions for this project. Loaded automatically by /ww-do.
...
5. **RULES.md location** — always `.ww-kit/RULES.md`, create `.ww-kit/` directory if needed
6. **Ownership boundary** — this command owns `.ww-kit/RULES.md`; other context artifacts stay read-only unless explicitly requested by the user
