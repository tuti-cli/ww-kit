...
```
Read .ww-kit/DESCRIPTION.md
```

Store project context for later steps. If absent, Step 2 detects everything.

**Read `.ww-kit/skill-context/wws-docker/SKILL.md`** — MANDATORY if the file exists.
...
Same detection as `/wws-build` Step 2.2.
...
Suggest follow-up: `/wws-build` for Docker targets, `/wws-docs` for documentation.
