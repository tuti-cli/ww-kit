...
3. Generating custom skills via `/wws-skill`
...
- If not found — ask the user via `AskUserQuestion`:
  ...
  3. Install Python first and re-run `/ww`
...
- "Install Python first" → **STOP**, user will re-run `/ww` after installing
...
- Never run blocking security decisions on built-in ww-kit skills (`~/{{skills_dir}}/ww` and `~/{{skills_dir}}/ww-*`).
- If the target path points to built-in `ww*` skills, treat it as wrong target selection and continue with the actual external skill path.

**Level 1 — Automated scan:**
```bash
$PYTHON ~/{{skills_dir}}/wws-skill/scripts/security-scan.py <installed-skill-path>
```
...
**Both levels must pass.** See [skill-generator CRITICAL section](../wws-skill/SKILL.md) for full threat categories.

---

### Project Context

**Read `.ww-kit/skill-context/ww/SKILL.md`** — MANDATORY if the file exists.
...
For each recommended skill:
  ...
  3. SECURITY: Scan installed EXTERNAL skill (never built-in ww*) → $PYTHON security-scan.py <path>
     - BLOCKED? → rm -rf <path>, warn user, skip this skill
     - WARNINGS? → show to user, ask confirmation
  4. If not found → Generate: /wws-skill <name>
  5. Has reference URLs? → Learn: /wws-skill <url1> [url2]...
```
...
**Trigger:** `/ww` (no arguments) + project has config files
...
**Step 2: Generate .ww-kit/DESCRIPTION.md**
...
1. Create directory: `mkdir -p .ww-kit`
2. Save `.ww-kit/DESCRIPTION.md`
3. For each external skill from skills.sh:
   ```bash
   npx skills install {{skills_cli_agent_flag}} <name>
   # AUTO-SCAN: immediately after install
   $PYTHON ~/{{skills_dir}}/wws-skill/scripts/security-scan.py <installed-path>
   ```
...
4. Generate custom skills via `/wws-skill` (pass URLs for Learn Mode when docs are available)
5. Configure MCP in `{{settings_file}}`
6. Generate `AGENTS.md` in project root (see [AGENTS.md Generation](#agentsmd-generation))
7. Generate architecture document via `/ww-arch` (see [Architecture Generation](#architecture-generation))

---

### Mode 2: New Project with Description

**Trigger:** `/ww <project description>`
...
**Step 2: Create .ww-kit/DESCRIPTION.md**
...
Install skills, configure MCP, generate `AGENTS.md`, and generate architecture document via `/ww-arch` as in Mode 1.

---

### Mode 3: Interactive New Project (Empty Directory)

**Trigger:** `/ww` (no arguments) + empty project (no package.json, composer.json, etc.)
...
**Step 3: Create .ww-kit/DESCRIPTION.md**
...
- Reference `.ww-kit/DESCRIPTION.md` for tech stack
...
- The Documentation section will be maintained by `/wws-docs`
...
- Primary ownership in this command: `.ww-kit/DESCRIPTION.md`, setup-time `AGENTS.md`, installed skills, and MCP configuration.
- Delegated ownership: invoke `/ww-arch` to create/update `.ww-kit/ARCHITECTURE.md`.
- Read-only context in this command by default: `.ww-kit/ROADMAP.md`, `.ww-kit/RULES.md`, `.ww-kit/RESEARCH.md`, and plan files.
...
Invoke `/ww-arch` to define project architecture. This creates `.ww-kit/ARCHITECTURE.md` with architecture pattern, folder structure, dependency rules, and code examples tailored to the project.
...
Project description: .ww-kit/DESCRIPTION.md
Architecture: .ww-kit/ARCHITECTURE.md
Project map: AGENTS.md
Skills installed: [list]
MCP configured: [list]

To start development:
- /wws-roadmap — Create a strategic roadmap with milestones (recommended for new projects)
- /ww-plan <description> — Plan implementation (creates branch + plan, or quick plan)
- /ww-do — Execute existing plan
...
- /wws-docs — Generate project documentation
- /ww-rules — Add project-specific rules and conventions
- /wws-build — Configure build scripts and automation
- /wws-ci — Set up CI/CD pipeline
- /wws-docker — Containerize the project

Would you like to run any of these now?
```

Present these as `AskUserQuestion` with multi-select options:
1. Generate docs (`/wws-docs`)
2. Build automation (`/wws-build`)
3. CI/CD (`/wws-ci`)
4. Dockerize (`/wws-docker`)
