---
name: ww-evolve
description: Self-improve ww-kit skills based on project context, accumulated patches, and codebase patterns. Analyzes what went wrong, what works, and enhances skills to prevent future issues. Use when you want to make AI smarter for your project.
...
Analyze project context, patches, and codebase to improve existing skills. Makes ww-kit smarter with every run.
...
1. **Raw patches** (`.ww-kit/patches/*.md`) are the source material.
2. **Skill-context rules** (`.ww-kit/skill-context/*`) are the compact, reusable output.

Policy across workflow skills:
- `/ww-evolve` is the primary raw-patch analyzer. It processes patches **incrementally** using a cursor.
- `/ww-do`, `/ww-fix`, and `/ww-improve` should prefer skill-context first; raw patches are fallback context only.
- Force full re-analysis only when needed (e.g., reset cursor and rerun evolve).

## Critical: Never Edit Built-in Skills Directly

**NEVER modify any files inside built-in `ww-*` skill directories** (`skills/ww-*/`).
All files in these directories are owned by ww-kit and will be overwritten on update — any direct edits will be lost.

**ALWAYS write project-specific rules to skill-context:**
```
.ww-kit/skill-context/<skill-name>/SKILL.md
```
...
Rule: first, strip any leading `/` from the argument. Then: if the argument does not start with `ww-` AND a skill named `ww-<argument>` exists — use `ww-<argument>`. Otherwise use as-is.
...
"Skill '<resolved-name>' not found. Use `/ww-evolve` without arguments to evolve
...
**Read `.ww-kit/DESCRIPTION.md`** to understand:
- Tech stack
- Architecture
- Conventions
...
- If evolving a **specific skill** (e.g., `/ww-evolve plan`) → read only:
  1. `.ww-kit/skill-context/ww-plan/SKILL.md` (target skill's context)
  2. `.ww-kit/skill-context/ww-evolve/SKILL.md` (evolve's own context, if exists
     **and** target skill is not `ww-evolve` itself)
- If evolving **all skills** (`/ww-evolve` or `/ww-evolve all`) → read all context files:
  `Glob: .ww-kit/skill-context/*/SKILL.md` (this already includes evolve's own context — do NOT read it separately)
...
**How to apply evolve's own skill-context rules (`ww-evolve`):**
...
```
Glob: .ww-kit/patches/*.md
```

Cursor file:

```
.ww-kit/evolutions/patch-cursor.json
```
...
7. Full rescan procedure: delete `.ww-kit/evolutions/patch-cursor.json`, then run `/ww-evolve` again.
...
(e.g., for `/ww-plan` — focus on file structure and naming; for `/ww-fix` — error handling and testing).

### Step 2: Read Target Skills

**Read ONLY the base SKILL.md files for target skills — not all skills.**

- If evolving a **specific skill** (e.g., `/ww-evolve plan`) → read only that one:
  `Read: {{skills_dir}}/ww-plan/SKILL.md` (or `skills/ww-plan/SKILL.md` if not installed)
- If evolving **all skills** (`/ww-evolve` or `/ww-evolve all`) → read all:
...
**When:** Run this step for every **target** `ww-*` skill that has a skill-context file.

**For each rule in `.ww-kit/skill-context/<skill-name>/SKILL.md`:**

**Scope constraint:** Step 3 can ONLY modify or remove skill-context files.
It must NEVER propose editing, deleting, or reverting base `skills/ww-*/` files.
...
Never propose editing, deleting, or reverting base `skills/ww-*/` files —
...
##### /ww-plan — Fully covered: [Rule Name]
...
##### /ww-fix — Partial overlap: [Rule Name]
- **Base SKILL.md says:** [base rule text]
- **Skill-context says:** [project rule text]
...
- **`skill-context`** → `.ww-kit/skill-context/<skill-name>/SKILL.md`
- **`SKILL.md`** → direct edit of the skill's own `SKILL.md` (only for custom/non-ww skills)
...
#### /ww-fix (N rules)
**Target:** `.ww-kit/skill-context/ww-fix/SKILL.md`
...
#### /ww-do (N rules)
**Target:** `.ww-kit/skill-context/ww-do/SKILL.md`
...
**If the skill is a built-in `ww-*` skill** (its SKILL.md is inside the package `skills/` directory):

1. Create directory if needed:
   mkdir -p .ww-kit/skill-context/<skill-name>
2. If `.ww-kit/skill-context/<skill-name>/SKILL.md` doesn't exist — create it with the header template
...
5. **NEVER edit any files inside the skill's `skills/ww-*/` directory** — all files there are owned by ww-kit and WILL be overwritten on update. All improvements go to skill-context only.
6. After applying all changes (including stale rule removals), if a skill-context file has no rules left
   ...
   `rm .ww-kit/skill-context/<skill-name>/SKILL.md`
   `rmdir .ww-kit/skill-context/<skill-name>` (only if empty)
...
**If the skill is a custom/project skill** (not `ww-*`):
...
> Auto-generated by `/ww-evolve`. Do not edit manually.
...
Create `.ww-kit/evolutions/YYYY-MM-DD-HH.mm.md`:

```bash
mkdir -p .ww-kit/evolutions
```
...
- [change description] ← driven by patches: [patch filenames]
  **File:** `.ww-kit/skill-context/[skill-name]/SKILL.md`
...
1. **Run `/ww-review`** on recent code to verify improvements
2. **Next evolution** — run `/ww-evolve` again after 5-10 more fixes
3. **Consider new skill** — if pattern X keeps recurring, create a dedicated skill:
   `/wws-skill <skill-name>`
```
...
7. **Skill-context only** — all improvements for built-in `ww-*` skills go to `.ww-kit/skill-context/`, never to `skills/ww-*/`. **NEVER edit any files inside `skills/ww-*/`** — they are overwritten on update. No exceptions.
...
10. **No new skills** — suggest `/wws-skill` instead
...
13. **Ownership boundary** — this command owns `.ww-kit/evolutions/*.md`, `.ww-kit/evolutions/patch-cursor.json`, and `.ww-kit/skill-context/*`; treat roadmap/rules/research/plan artifacts as read-only context unless explicitly asked
...
→ Improvement for /ww-fix (2 rules):
  Target: .ww-kit/skill-context/ww-fix/SKILL.md
