#!/bin/bash
# Smoke tests: validates ww-kit update status model and --force behavior
...
cat > "$PROJECT_DIR/.ww-kit.json" << 'EOF'
{
  "version": "2.4.0",
  "agents": [
    {
      ...
      "installedSkills": ["ww", "ww-plan", "ww-nonexistent"],
      "mcp": {
        "github": false,
        "filesystem": false,
        "postgres": false,
...
assert_contains "$FIRST_OUTPUT" "ww-nonexistent \(removed from package\)" "removed package skill must be reported"

# Managed state should be persisted after first run.
node -e "const fs=require('fs');const c=JSON.parse(fs.readFileSync(process.argv[1],'utf8'));const m=c.agents[0].managedSkills||{};if(!m['ww']||!m['ww-plan']){process.exit(1);}" "$PROJECT_DIR/.ww-kit.json"
...
cat > "$CLAUDE_PROJECT_DIR/.ww-kit.json" << 'EOF'
{
  "version": "2.4.0",
  "agents": [
    {
...
assert_contains "$CLAUDE_FIRST_OUTPUT" "Subagents:" "claude subagents section must be printed"
